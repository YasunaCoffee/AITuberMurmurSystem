...
from murmur.core.events import NewCommentReceived
...
class TestIntegratedCommentManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher_pytchat = patch(
            'murmur.services.integrated_comment_manager.pytchat'
        )
        self.mock_pytchat = self.patcher_pytchat.start()
        
        self.mock_chat = MagicMock()
        self.mock_chat.is_alive.return_value = True
        ...
        # start() を経由しないテストでは chat を明示（create は start 時のみ呼ばれる）
        self.comment_manager.chat = self.mock_chat
